...
intercept_cases <- global %>% summarize (rate = sum(prevalence)/sum(test_per_million)) %>% pull(.)

global_cases_plot <-
ggplot(global) + geom_text(aes(x = test_per_million, y = prevalence, label = alpha3), size = 3) +
  scale_x_continuous(trans = "log10") +
  scale_y_continuous(trans = "log10") +
  geom_abline(intercept = log10(intercept_cases)) +
  labs(title = "Relación número de pruebas Vs prevalencia",x = "Casos por cada millón", y = "Prevalencia")
```
