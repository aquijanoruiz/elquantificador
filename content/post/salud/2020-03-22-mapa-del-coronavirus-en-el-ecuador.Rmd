...
confirmed_p <- confirmed_p %>% gather(Fecha, Casos, -Provincia) %>% 
  mutate(Fecha = as.Date(Fecha, format = "%m/%d/%Y")) %>%
  filter(Fecha >= as.Date("2020-03-05")) %>% # We only have data by province from May 16th
  replace_na(list(Casos = 0))
...
covid19_nozeros_p <- covid19_confirmed_p[!covid19_confirmed_p$Casos == 0,]

covid19_confirmed_today_map <-
  tm_shape(ecu_map_p) + tm_borders(col = "grey", lwd = 2, alpha = 0.4) +
  tm_polygons(col = "skyblue", alpha = 0.2) + 
  tm_shape(covid19_nozeros_p) + # We add a second tm_shape with the coronavirus data
  tm_fill(col = "Casos", alpha = 0, legend.show = FALSE) +
  tm_shape(covid19_nozeros_c) + # We add a second tm_shape with the coronavirus data
...
covid19_confirmed_today_map %>% removeLayersControl() %>%
  setView(lng = -78.50374, lat = -1.289527, zoom = 6) %>% fitBounds(-80.0, -4.2, -77.0, 0.8) # We center the map in Tungurahua
```
